...
def get_stdev_from_anchor(means, stdevs, measures):
    i = 0
    results = {}
    for m in measures.keys():
        results[m] = {}
        for k, value in measures[m].items():
            # if the difference is too small to be measured say it's zero, issue with floating points
...
def main():
    ...
    results = get_stdev_from_anchor(means, stdevs, cts_measures)
